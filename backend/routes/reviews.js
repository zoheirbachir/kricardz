...
router.post('/', auth, (req, res) => {
  ...
  if (Number(rating) < 1 || Number(rating) > 5) return res.status(400).json({ error: 'Note entre 1 et 5' });

  /* Only someone who actually rented this car (confirmed/completed booking) may
     review it — prevents fake/spam reviews from users who never used the vehicle. */
  const booking = db.prepare(
    "SELECT id FROM bookings WHERE car_id = ? AND renter_id = ? AND status IN ('confirmed','completed') ORDER BY created_at DESC LIMIT 1"
  ).get(car_id, req.user.id);
  if (!booking) return res.status(403).json({ error: 'Vous devez avoir loué ce véhicule pour laisser un avis.' });
  ...
  db.prepare('INSERT INTO reviews (id, car_id, reviewer_id, booking_id, rating, comment) VALUES (?, ?, ?, ?, ?, ?)').run(
    id, car_id, req.user.id, booking_id || booking.id, Number(rating), comment || null
  );
