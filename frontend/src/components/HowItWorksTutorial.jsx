...
function AccountScreen() {
  return (
    <div className="p-4 space-y-3">
      <p className="text-[13px] font-semibold text-gray-900">Créer un compte</p>
      {['Nom complet', 'Téléphone', 'Mot de passe'].map((l, i) => (
        <div key={l}>
      ...
      <motion.div
        ...
        <div>
          <p className="text-[10px] font-semibold text-pine-700 leading-tight">Identité vérifiée</p>
          <p className="text-[8px] text-pine-600 leading-tight">Pièce d'identité validée</p>
        </div>
      </motion.div>
      <div className="h-8 rounded-lg bg-primary-600 text-white text-[10px] font-semibold flex items-center justify-center">Créer mon compte</div>
    </div>
  );
}
  ...
  return (
    <div className="p-4 space-y-3">
      <div className="flex items-center gap-2 h-8 rounded-lg bg-gray-100 ring-1 ring-gray-200 px-2.5">
        ...
        <span className="text-[10px] text-gray-500">Wilaya · Type · Budget</span>
      </div>
      {cars.map((c, i) => (
        <motion.div key={c.n}
          ...
          <div className="text-[10px] font-bold text-primary-600 shrink-0">{c.p}<span className="text-[7px] text-gray-400"> DA/j</span></div>
        </motion.div>
      ))}
    </div>
  );
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="space-y-1.5">
        <div className="flex justify-between text-[9px]"><span className="text-gray-400">Du</span><span className="font-semibold text-gray-700">12 juin</span></div>
        <div className="flex justify-between text-[9px]"><span className="text-gray-400">Au</span><span className="font-semibold text-gray-700">15 juin</span></div>
        <div className="flex justify-between text-[10px] pt-1 border-t border-gray-100"><span className="text-gray-500">Total</span><span className="font-bold text-primary-600">27 000 DA</span></div>
      </div>
      <motion.div
        ...
        <p className="text-[10px] font-semibold text-pine-700">Réservation confirmée — récupérez les clés !</p>
      </motion.div>
    </div>
  );
}
...
const STEPS = [
  { n: '01', label: 'Créez votre compte', desc: 'Inscrivez-vous et faites vérifier votre identité.', Screen: AccountScreen },
  { n: '02', label: 'Trouvez une voiture', desc: 'Filtrez par ville, type et budget.', Screen: SearchScreen },
  { n: '03', label: 'Réservez & roulez', desc: 'Confirmez, récupérez les clés, partez.', Screen: BookScreen },
];
