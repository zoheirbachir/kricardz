...
export default function AdminKyc() {
  ...
  const infoRows = detail ? [
    ...
    ['Type de document', detail.document_type === 'id_card' ? "Carte d'identité" : detail.document_type === 'driving_license' ? 'Permis de conduire' : detail.document_type === 'passport' ? 'Passeport' : detail.document_type],
    ['N° de document', detail.document_number],
    ['N° de permis de conduire', detail.driving_license_number],
    ...
    ["Adresse de l'agence", detail.agency_address],
    ["NIN du gérant / N° d'identité nationale", detail.national_id_number],
              ...
              {presentDocs.length === 0 ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {presentDocs.map(([k, label]) => (
                    <AuthDoc key={k} path={docs[k]} label={label} />
                  ))}
                </div>
              )}
