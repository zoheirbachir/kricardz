...
import api from '../api';
import { AGENCY_TYPE_LABELS as AGENCY_TYPES } from '../lib/agencyTypes';
...
export default function Agencies() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        <p className="eyebrow mb-1.5">Réseau partenaire</p>
        <h1 className="section-title mb-2">Agences de location</h1>
        <p className="text-gray-500">Découvrez les agences de confiance pour la location de voitures.</p>
      </div>
      ...
      <div className="card p-5 mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Rechercher par nom d'agence</label>
            <input className="input text-sm" placeholder="Nom de l'agence..." value={f.search} onChange={e => set('search', e.target.value)} />
          </div>
          <div>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Type d'agence</label>
            <select className="input text-sm" value={f.type} onChange={e => set('type', e.target.value)}>
              <option value="">Tous les types</option>
              {Object.entries(AGENCY_TYPES).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
            </select>
          </div>
          <div>
            ...
            <select className="input text-sm" value={f.sort} onChange={e => set('sort', e.target.value)}>
              <option value="vehicles">Plus de véhicules</option>
      ...
      {!loading && (
        <p className="text-sm text-gray-500 mb-4">
          {filtered.length} {filtered.length > 1 ? 'agences trouvées' : 'agence trouvée'}
        </p>
      )}
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-gray-500">Aucune agence ne correspond à votre recherche</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              ...
              <Link to={`/agencies/${agency.id}`}
                ...
                <div className="pt-9 px-5 pb-5">
                  ...
                  <span className="badge bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 mt-3">
                    {AGENCY_TYPES[agency.agency_type] || AGENCY_TYPES.classic}
                  </span>
