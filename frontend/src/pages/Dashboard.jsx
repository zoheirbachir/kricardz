    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div className="min-w-0">
          <h1 className="font-display text-2xl sm:text-3xl font-semibold tracking-tight text-gray-900 truncate">Bonjour, {user?.name?.split(' ')[0]}</h1>
          <p className="text-gray-500 text-sm mt-1">{t('nav.myBookings')}</p>
        </div>
        {(user?.role === 'owner') && (
          <Link to="/dashboard/owner" className="btn-primary text-sm self-start shrink-0">Mes annonces
      ...
      {/* Documents whose file no longer exists on the server — the user must
          send them again before their account can be verified. */}
      {user?.kyc_files_missing?.length > 0 && (
        <Reveal>
          <div className="mb-6 rounded-2xl border border-red-200 bg-red-50 dark:bg-red-500/10 dark:border-red-500/30 p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="font-semibold text-red-700 dark:text-red-300">Vos documents d'identité doivent être renvoyés</p>
                <p className="text-sm text-red-600/90 dark:text-red-300/80 mt-0.5 leading-relaxed">
                  Suite à un incident technique sur nos serveurs, {user.kyc_files_missing.length} de vos documents
                  ne sont plus disponibles. Merci de les téléverser à nouveau — nos excuses pour la gêne occasionnée.
                </p>
                <div className="mt-4">
                  <KycReupload missing={user.kyc_files_missing} />
                </div>
              </div>
            </div>
          </div>
      ...
      {user?.kyc_status === 'pending' && !user?.kyc_files_missing?.length && (
        <Reveal>
          <div className="mb-6 rounded-2xl border border-honey-200 bg-honey-50 dark:bg-honey-500/10 dark:border-honey-500/30 p-4 flex items-start gap-3">
            <div className="w-10 h-10 rounded-xl bg-honey-100 dark:bg-honey-500/20 flex items-center justify-center text-honey-700 dark:text-honey-200 shrink-0">
        ...
        <Reveal>
          <div className="mb-6 rounded-2xl border border-red-200 bg-red-50 dark:bg-red-500/10 dark:border-red-500/30 p-4 flex items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              {/* A refusal is fixable — let them send corrected documents. */}
              <div className="mt-4">
                <p className="text-xs font-semibold uppercase tracking-wide text-red-700/80 dark:text-red-300/70 mb-2">Envoyer de nouveaux documents</p>
                <KycReupload />
              </div>
            </div>
          </div>
